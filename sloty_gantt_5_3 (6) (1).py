...
def save_state_to_json(filename="schedules.json"):
    ...
    logging.info("Stan zapisany do %s", filename)
...
def load_state_from_json(filename="schedules.json"):
    ...
    logging.info("Stan załadowany z %s", filename)
    return True
    ...
    for line in text.splitlines():
        ...
        if len(parts) >= 3:
            try:
                ...
                logging.warning("Błąd parsowania slotu: %s", e)
        elif len(parts) == 2:
            try:
                out.append({"name": parts[0].strip(),
                            "minutes": int(parts[1]),
...
def schedule_client_immediately(client_name, slot_type_name, day, pref_start, pref_end):
    ...
    if not candidates:
        return False, None
    candidates.sort(key=lambda x: (x[1], len(st.session_state.schedules[x[0]].get(day.strftime("%Y-%m-%d"), []))))
